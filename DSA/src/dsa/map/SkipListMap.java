...
public class SkipListMap<K extends Comparable<K>, V> extends AbstractOrderedMap<K, V> {
    ...
    public V get(K key) {
        ...
        if(temp == null || temp.getEntry() == null || isSentinel(temp) || !temp.getEntry().getKey().equals(key)) {
        	return null;
        } else {
    ...
    public V put(K key, V value) {
        ...
        //!isSentinel(p) && 
        if (!isSentinel(p) && p.getEntry().getKey().equals(key)) {
        	V originalValue = p.getEntry().getValue();
        	while (p != null) {
        		MapEntry<K, V> add = (MapEntry<K, V>) p.getEntry();
        		add.setValue(value);
        		p = p.getAbove();
    ...
    public V remove(K key) {
        ...
        //if node was not found or does not exist
        if (isSentinel(temp) || temp == null || temp.getEntry() == null || !temp.getEntry().getKey().equals(key)) {
        	return null;
        }
        
        V ret = temp.getEntry().getValue();
        //if node was found go to top of the list
//        while (temp.getAbove() != null) {
//        	temp = temp.getAbove();
//        }
        
        //Delete node from row
        while(temp != null) {
        	
        	temp.getPrevious().setNext(temp.getNext());
        	temp.getNext().setPrevious(temp.getPrevious());
        	//(MapEntry<K, V>) temp.getElement().setKey(null);
        	//((MapEntry<K, V>) temp.setValue(value);
        	//((MapEntry<K, V>) temp.getEntry()).setValue(null);
        	//temp.setEntry(null);
        	//((MapEntry<K, V>) temp.getEntry()).setKey(null);
        	temp = temp.getAbove();
        }
        
        //Removing empty rows
        if (start.below.getNext() == null) {
        	start = start.below;
        	height--;
        }
        size--;
...
//    }
    ...
    /**
     * The Node class to define the characteristics of a skiplist node
     * @author 316 Instructor Dr. Sturgil from workshop
     * Node class
     * @param <K> key
     * @param <V> value
     */
