...
//    	return -1;
    ...
    private int binarySearchHelper(int min, int max, K key) {
        ...
        if (list.get(mid).getKey().equals(key)) {
        	return mid;
        } else if (compare(list.get(mid).getKey(), key) > 0) { //possible issue
    ...
    public V put(K key, V value) {
        ...
        int i;
        if (index < 0) {
        	i = binarySearchHelper(0, list.size() - 1, key);
        	list.add(-1 * (i + 1), new MapEntry<K, V>(key, value));
        	return null;
        } else {
        	V ret = list.get(index).getValue();
        	MapEntry<K, V> replace = (MapEntry<K, V>) list.get(index);
